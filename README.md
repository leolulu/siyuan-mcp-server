...
该服务器充当一座桥梁，将 MCP 的工具调用转换为对思源笔记 API 的请求，专注于提供强大的只读查询能力，并为后续写入能力提供统一的前台通知基础。
...
- **前台通知工具**: 提供 `push_message` 和 `push_error_message`，用于非只读流程的结果反馈与错误提示。
...
-   **`push_message`**: 推送前台普通消息（用于写入/创建/修改等操作结果提示）。
-   **`push_error_message`**: 推送前台错误消息（用于写入流程中的异常提示）。
-   **`create_document`**: 通过 Markdown 创建文档（写入操作，内置成功/失败通知）。
-   **`update_block`**: 更新指定块内容（写入操作，内置成功/失败通知）。
-   **`delete_block`**: 删除指定块（写入操作，内置成功/失败通知）。
-   **`insert_block`**: 在指定锚点位置插入块（写入操作，内置成功/失败通知）。
-   **`prepend_block`**: 插入前置子块（写入操作，内置成功/失败通知）。
-   **`append_block`**: 插入后置子块（写入操作，内置成功/失败通知）。
-   **`move_block`**: 移动块到指定位置（写入操作，内置成功/失败通知）。默认按“逻辑块组”执行，避免父块与内容脱离（标题按分节范围，其它块按子树后代）。
...
为后续写入/创建/修改能力做准备，建议在非只读操作中统一使用通知工具：
